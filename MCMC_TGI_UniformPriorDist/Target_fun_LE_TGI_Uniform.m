function Target_value = Target_fun_LE_TGI_Uniform(theta,Model_Index)
...
if Model_Index  == 1
    prior_T_d_Tv                =  unifpdf(theta(1), 100,1000) ;%normpdf(theta(1),500,100);
    prior_IC50                       = unifpdf(theta(2), 0.1,50);  %wblpdf(theta(2),10,2);
    prior_E_max_damage   =  unifpdf(theta(3),1e-8,1);
    prior_EC_50_damage      =   unifpdf(theta(4),1e-8,20); %wblpdf(theta(4),5.2,2);
    prior_g                         = unifpdf(theta(5),1e-8,1);%1e-3
    prior_Pmax                      = unifpdf(theta(6),0.1, 20); % wblpdf(theta(6),5,2);
    ...
    prior_nosie_treated     =   unifpdf(ThetaNoise(1), 0.001,  10); %gampdf( ThetaNoise(1), 1,1 );
    prior_nosie_control     =  unifpdf(ThetaNoise(2), 0.001, 10); %gampdf( ThetaNoise(2), 1,1 );
...
elseif Model_Index  == 2
    ...
    prior_E_max_damage   =   unifpdf(theta(3),0.1, 50); %%wblpdf(theta(3),22,2) ;
    prior_EC_50_damage      = unifpdf(theta(4),1,400);  % wblpdf(theta(4),157,2);
    prior_g                         = unifpdf(theta(5),1e-8,1);%1e-3
...
elseif Model_Index  == 3
    ...
    prior_T_d_Tv                =  unifpdf(theta(1),150,1500);  %normpdf(theta(1),900,100);
    ...
    prior_g                         = unifpdf(theta(5),1e-8,1);%1e-3
    prior_Pmax                      =  unifpdf(theta(6),0.5,5); %1e-3 %wblpdf(theta(6),3,2);
    prior_d                         = unifpdf(theta(7),1e-8,1); %1e-3
    % prior_nosie_treated     =  lognpdf(ThetaNoise(1),4,0.4);
    % prior_nosie_control     =    gampdf( ThetaNoise(2),4,3 );
    prior_nosie_treated     =  unifpdf(ThetaNoise(1), 0.001,10); %gampdf( ThetaNoise(1), 1,1 );
...
elseif Model_Index  == 5
    % Treated for 20 mg/kg Twice a week for four weeks
    prior_T_d_Tv                =   unifpdf(theta(1),150,1500); % normpdf(theta(1),800,100);
    prior_IC50                       =  unifpdf(theta(2),1e-8,2); %enlarge from 1
    prior_E_max_damage   =  unifpdf(theta(3),1e-8,2);
    prior_EC_50_damage      = unifpdf(theta(4),1e-8,2);
    prior_g                         = unifpdf(theta(5),1e-8,1);%1e-3
    ...
    % prior_nosie_treated     =  lognpdf(ThetaNoise(1),4,0.4);
    % prior_nosie_control     =    gampdf( ThetaNoise(2),4,3 );
    prior_nosie_treated     =  unifpdf(ThetaNoise(1), 0.0001,10); %gampdf( ThetaNoise(1), 1,1 );
    prior_nosie_control     =  unifpdf(ThetaNoise(2), 0.001,10); %gampdf( ThetaNoise(2), 1,1 );
elseif Model_Index  == 6
    ...
    prior_EC_50_damage = 1;
    prior_g                         = unifpdf(theta(1),1e-8,1);%1e-3
    prior_Pmax                      =  unifpdf(theta(2),1e-2,50);%  %wblpdf(theta(2),13,2) ; %unifpdf(theta(2),8,16); %1e-3 %wblpdf(theta(6),3,2);
    prior_d                         = unifpdf(theta(3),1e-8,1); %1e-3
    prior_nosie_treated     =  unifpdf(ThetaNoise(1), 0.001,10); %gampdf( ThetaNoise(1), 1,1 );
    prior_nosie_control     =  unifpdf(ThetaNoise(2), 0.001,10); %gampdf( ThetaNoise(2), 1,1 );
...
elseif Model_Index  == 8
    prior_T_d_Tv                =  unifpdf(theta(1),150,1500); % normpdf(theta(1),800,100);
    prior_IC50                       = unifpdf(theta(2),0.5,500); % % normpdf(theta(2),50,100);
    prior_E_max_damage   = unifpdf(theta(3),1e-8,1);
    prior_EC_50_damage    = unifpdf(theta(4),1e-8,100);
    prior_g                         = unifpdf(theta(5),1e-8,1);%1e-3
    prior_Pmax                      =  unifpdf(theta(6),1e-2,20); % wblpdf(theta(6),5,2) ;
    prior_d                         = unifpdf(theta(7),1e-8,1); %1e-3
    prior_nosie_treated     =  unifpdf(ThetaNoise(1), 0.001,10); %gampdf( ThetaNoise(1), 1,1 );
    prior_nosie_control     =  unifpdf(ThetaNoise(2), 0.001,10); %gampdf( ThetaNoise(2), 1,1 );

...
function  Plot_TGIModel(theta_col, theta, Model_Index)
...
p1 = plot(Tsim,Ysim(:,1) ,'LineWidth',2,'color', Color(1,:));  % treated group 
hold on;
[ha1 hb hc] = shadedplot(Tsim', transpose(CI_treated(:,1)) ,transpose(CI_treated(:,2)) , [17 17 17]/255,'none'); % confidence interval
hold on
p2 = plot(Tsim,Ysim(:,2) ,'-.', 'LineWidth',2, 'color',Color(2,:)) ; % control group;  different line style than the treated group
hold on
...
legend([p1 ha1 p2 ha2 e1 e2 s],{'simulation-treated group','','Prediction interval for treated group', 'simulation-control group','','Prediction interval for control group',...
...
for iFig = 1:length(FigList)
    ...
    exportgraphics(FigHandle,fullfile(newdir , [FigName '.tif']), 'Resolution', 300 )
end
end
function  [T_TimeCourse_final,Cv_UPtoFreeTS_Timecourse_2] = RepeatedDoses_kinetics_Cellular3to10(theta, Dose_info, Control_flag, scatter_injection,Dose_order)


Dose_col = Dose_info(:,end); num_Dose_type  = length(Dose_col);

if Control_flag == 1
    Dose_col  = zeros( num_Dose_type,1);
end
...
for indi_injection = 1: num_injection
     if indi_injection < num_injection
            ...
            % For example , indi_injection= 1,num_Dose_type = 2, Dose_idx= 1
            [Tv,Cv]=ode15s(@DifEq, indi_dur,c0);
            Dose_idx = Dose_order( indi_injection+1 );
            Dose  = Dose_col(    Dose_idx ) ;

            Tv= Tv';
            Cv(Cv<=0) =0;
            c0(1) = Dose+ Cv(end,1); %reinitialize
            c0(2:end) = Cv(end,2:end); %reinitialize
            Tv_TimeCourse( end+1 : end+size(Tv,2)  )  = Tv + Tv_TimeCourse(end);
            Cv_complex_Timecourse( end+1: end+size(Tv,2) ,:  ) =  Cv;
     elseif indi_injection == num_injection %circument the situation where Duration=Interval_TotalNum*DoseFrequency
        T_RNA = [];
        Cv_DNA  = [];
        Cv_RNA = [];
        T_DNA = [];

function  [T_TimeCourse_final,Cv_UPtoFreeTS_Timecourse_2] = RepeatedDoses_kinetics_Cellular3to10(theta, Dose_info, Control_flag, scatter_injection)
...
[TotalDuration,  Interval_TotalNum_PerCycle ,DoseFrequency , TreatmentDuration_PerCycle,    num_cycle , Dose ] = Dose_info_cell{:};
c0=[Dose
    0
    0
    0
...
for indi_cycle = 1:num_cycle
    if Interval_TotalNum_PerCycle> 0
        for i =1:1: Interval_TotalNum_PerCycle
    ...
    if  TreatmentDuration_PerCycle  >  Interval_TotalNum_PerCycle*DoseFrequency
        T_RNA = [];
        Cv_DNA  = [];
        Cv_RNA = [];
        T_DNA = [];
        count_genome = 1;
        t_interval_final = [0    TreatmentDuration_PerCycle - Interval_TotalNum_PerCycle*DoseFrequency  ];
        %t_interval_final = 0 :100:   Duration - Interval_TotalNum*DoseFrequency ;
        %options = odeset('RelTol', 2.22045e-2,'AbsTol',1e-2);
        [Tv,Cv]=ode15s(@DifEq,t_interval_final,c0); %15s
        
        Tv = Tv';
        Cv(Cv<=0) =0;
        Tv_TimeCourse( end+1 : end+size(Tv,2)  )  = Tv +Interval_TotalNum_PerCycle*DoseFrequency; %lump the time courses of interdose intervals
        Cv_complex_Timecourse( end+1: end+size(Tv,2),: ) =  Cv ;
    end
end

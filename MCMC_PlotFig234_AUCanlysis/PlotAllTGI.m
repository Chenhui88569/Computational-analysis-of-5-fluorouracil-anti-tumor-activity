close all
clear
portion_coll = [0.2; 0.15;0.15;0.55;0.4;0.4;0;0.3;0.15 ];
...
if ~isfolder(newdir_parent  )
    ...
    rmdir(  newdir_parent ,'s'  )
    mkdir( newdir_parent   ) ;
    %delete(  fullfile(newdir_parent  ,'*' )   )
end
for Model_Index = 1:9
...
%     end
    ...
    for iFig = 1: ceil(num_para/factor )
        ...
        sgtitle(['Histogram for parameters in model'   num2str(Model_Index)],'FontSize', 18)
        saveas( f, fullfile(newdir_parent  , ['hist'  num2str(iFig) '_Model' num2str(Model_Index) '.tif']));

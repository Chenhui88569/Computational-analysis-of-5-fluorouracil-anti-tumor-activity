function Target_value = Target_fun_LE_Cellular_Uniform(theta)
...
%% %5-FU in interstitial fluid
prior_Q31 =  unifpdf( theta(1),0.1,10); %wblpdf(theta(1) ,1,2); 
prior_Vinflux =  unifpdf( theta(2),100,1000);  %normpdf(theta(2) , 5.4e2,50);
prior_Kinflux =  unifpdf( theta(3),40,400); %normpdf(theta(3) , 1.71e2,20);
prior_Vefflux =  unifpdf( theta(4),100,1000);%normpdf(theta(4) , 5.37e2,50);
prior_Kefflux = unifpdf( theta(5),0.1,20);  %normpdf(theta(5) , 6.35,3);
prior_k03 =   unifpdf( theta(6),25,250); %normpdf( theta(6), 1.27e2,20);%chi2pdf(theta(9:10),1);
prior_V54 =    unifpdf( theta(7),0.1,100); % ;normpdf(theta(7), 2.52e1,10);
prior_K54 = unifpdf( theta(8),500,5000); %normpdf(theta(8), 2.32e3,50);
%% 5FU incorporation into DNA and RNA
%prior_k_65 = gampdf(theta(9), 1,1 )  ;
prior_k_65 = unifpdf( theta(9),1e-3,0.8) ;
%prior_k_56 = gampdf(theta(10), 1,1 )  ;
...
%prior_gamma_RNA = gampdf(theta(12), 1,1 ) ;
prior_gamma_RNA = unifpdf(theta(12), 1e-5,0.8) ;
prior_V_75 =  unifpdf( theta(13),1e-3,10); %gampdf(theta(13), 1,1);
prior_K_75 =   unifpdf( theta(14),1e-3,10); %gampdf(theta(14), 1,1);
prior_V_57 =  unifpdf( theta(15),0.1,20); % wblpdf(theta(15), 5,2);
prior_K_57 = unifpdf( theta(16),0.05,5); % wblpdf(theta(16), 1,2);
prior_k07   =  unifpdf( theta(17),0.05,5); % wblpdf(theta(17), 1,2);
prior_gamma_DNA = unifpdf( theta(18),0.05,5); % wblpdf(theta(18), 1,2);
prior_T_RNA = unifpdf( theta(19),0.05,5); %wblpdf(theta(19), 1,2);
prior_T_DNA = unifpdf( theta(20),0.1,20); % wblpdf(theta(20), 5,2);
prior_k95 = unifpdf( theta(21),1e-3,10); % gampdf( theta(21), 1,1       );
% prior_k95  = unifpdf( theta(21),1e-3,0.2) ;
prior_k59 =  unifpdf( theta(22),0.1,20); %wblpdf( theta(22), 5,2       );
prior_k09 =   unifpdf( theta(23),1e-3,10);  %gampdf( theta(23), 1,1     );

prior_KdUMP = unifpdf( theta(24),0.1, 100); %wblpdf( theta(24), 17,2    );
prior_G0 =  unifpdf( theta(25),1e-3,10); %gampdf( theta(25), 1,1    );
prior_kcat  = unifpdf( theta(26),0.1,100);   %wblpdf( theta(26), 18,2  );
%prior_k08 = gampdf( theta (27),1,1 ) ;
prior_k08 = unifpdf( theta(27),1e-3,0.8) ;

prior_noise_intra    = unifpdf( ThetaNoise(1) ,5,500) ;    % lognpdf(ThetaNoise(1),4,0.4);
prior_nosie_FNUC  =  unifpdf( ThetaNoise(2) ,5,500) ;  % lognpdf(ThetaNoise(2),4,0.4); 
prior_nosie_RNA   =   unifpdf( ThetaNoise(3) ,1e-3,20) ; %  gampdf( ThetaNoise(3),4,1 );
prior_nosie_DNA   =   unifpdf(ThetaNoise(4),2.0093,10); %gampdf( ThetaNoise(4), 1,1 );
prior_nosie_TS      =   unifpdf( ThetaNoise(5),0.20315,10);   %gampdf( ThetaNoise(5), 1,1 );
prior_nosie_dUMP =    unifpdf(  ThetaNoise(6), 2,20 ); %lognpdf(ThetaNoise(6),1,0.5); 
prior_vec = [
    prior_Q31 
    prior_Vinflux
    prior_Kinflux 
...
for i = 1:num_dataset
    ...
    Noise_Term = ThetaNoise(i); % Noise_Term is the variance of the measurement noise.

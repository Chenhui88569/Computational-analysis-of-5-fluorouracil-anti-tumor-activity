function  [Tsim, Ysim,Ysim_interp] = kinetics_TGI(theta, Model_Index,T) 
%Usages:
...
Control_flag = 0;
[T_sim_treated, Cv_anabolites_treated,Cv_DSB_treated]  = RepeatedDoses_kinetics_uptoDSBandFreeTS(theta,Model_Index, Control_flag );
Control_flag = 1;
[T_sim_control, Cv_anabolites_control,Cv_DSB_control]  = RepeatedDoses_kinetics_uptoDSBandFreeTS(theta, Model_Index ,Control_flag);
...
theta_TGI = theta(end-6:end);
theta_effect = theta_TGI(1:4);
theta_ControlGroupPara = theta_TGI(5:end);

%options = odeset('RelTol',1e-4,'AbsTol',1e-6);

[Tsim,Cv]=ode15s(@fun_tumor_apop, [0 Duration] , c0_Tv ); %time span
%[Tsim,Cv]=ode15s(@fun_tumor_apop, T , c0_Tv ); %time span
...
Ysim_interp = interp1( Tsim,Ysim,T, 'PCHIP' );

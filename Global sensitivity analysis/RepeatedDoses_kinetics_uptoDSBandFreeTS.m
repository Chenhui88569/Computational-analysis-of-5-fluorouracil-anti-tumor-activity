function [T_UptoFreeTS,Cv_anabolites,Cv_DSB] = RepeatedDoses_kinetics_uptoDSBandFreeTS(theta, Model_Index,Control_flag)
...
theta_DSB_dNTP = theta(33:33+18);
[T_UptoFreeTS , C_sim_UpToFreeTS ] = RepeatedDoses_kinetics_Cellular3to10(theta_cellular_PK,  Model_Index, Control_flag);
PercentagefreeTS = C_sim_UpToFreeTS(:,end);

[T_DSB,Cv] = ode15s(@fun_DSB,[0 Duration] ,c0 ); %time span
Cv_DSB = Cv(:,3);
Cv_anabolites = C_sim_UpToFreeTS(:,5)  ;
Cv_DSB_q= interp1(T_DSB , Cv_DSB, T_UptoFreeTS,  'PCHIP');
Cv_DSB = real(Cv_DSB_q);
    ...
    function dC = fun_DSB(time_ode,c)
        ...
        theta_dNTP = theta_DSB_dNTP(1:12);
        k1  =  theta_dNTP(1);
        k2 = theta_dNTP(2);
        k3 = theta_dNTP(3);
        k4 = theta_dNTP(4);
        ...
        Day2min = 24*60;
        theta_DSB = theta_DSB_dNTP(13:end); 
